...
<div class="filemanager fp-mkdir-dlg" role="dialog" aria-live="assertive" aria-labelledby="fp-mkdir-dlg-title">
    ...
    // START OF HACK
    // Check if this is specifically for course image by URL
    $is_course_image = false;
    if (isset($_SERVER['REQUEST_URI']) && 
        strpos($_SERVER['REQUEST_URI'], '/course/edit.php') !== false &&
        isset($fm->options->context) && 
        $fm->options->context instanceof core\context\course &&
        isset($fm->options->maxfiles) && $fm->options->maxfiles == 1) {
        $is_course_image = true;
    }
    
    // Check if user is a teacher (not admin)
    $is_teacher_not_admin = false;
    if ($is_course_image) {
        global $USER;
        $is_admin = is_siteadmin($USER->id);
        $context = $fm->options->context;
        $has_edit_capability = has_capability('moodle/course:update', $context);
        
        $is_teacher_not_admin = $has_edit_capability && !$is_admin;
    }
    
    // Force 5MB display only for teachers (not admins)
    if ($is_teacher_not_admin && ($fm->options->maxbytes == 0 || $fm->options->maxbytes == -1 || $fm->options->maxbytes > 5242880)) {
        $maxbytes = display_size(5242880, 0); // 5MB in bytes
    } else {
     ...
     // END OF HACK
     // Removed this: $maxbytes = display_size($fm->options->maxbytes, 0);
...
//     // Force 5MB display only for course images
//     if ($is_course_image && ($fm->options->maxbytes == 0 || $fm->options->maxbytes == -1)) {
//         $maxbytes = display_size(5242880, 0); // 5MB in bytes
//     } else {
//         $maxbytes = display_size($fm->options->maxbytes, 0);
//     }
