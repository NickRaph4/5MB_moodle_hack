...
// Custom validation: if this is a .jpg, .jpeg, or .png and used for course image, limit to 5MB
$extension = strtolower(pathinfo($record->filename, PATHINFO_EXTENSION));

if (in_array($extension, ['jpg', 'jpeg', 'png'])) {
    $referer = $_SERVER['HTTP_REFERER'] ?? '';
    if (strpos($referer, '/course/edit.php') !== false) {
        global $USER;
        
        // Only apply 5MB limit to teachers, not admins
        $is_admin = is_siteadmin($USER->id);
        
        if (!$is_admin) {  // Only restrict teachers, not admins
            $maximgsize = 5 * 1024 * 1024; // 5MB
            if ($filesize > $maximgsize) {
                throw new file_exception('maxbytesfile', (object)[
                    'file' => $record->filename,
                    'size' => display_size($maximgsize, 0)
                ]);
            }
        }
    }
    ...
    // this works for admin and teacher too
        // // Custom validation: if this is a .jpg, .jpeg, or .png and used for course image, limit to 5MB (WORKS FINE. DONT FCKIN' TOUCH IT)
        // $extension = strtolower(pathinfo($record->filename, PATHINFO_EXTENSION));
